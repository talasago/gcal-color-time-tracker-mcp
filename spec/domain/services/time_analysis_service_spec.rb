  ...
  describe '#analyze' do
    context 'with comprehensive mixed events' do
      let(:filtered_events) do
        [
          ...
          EventFactory.timed_event(summary: '緑2', color_id: EventFactory::GREEN, duration_hours: 2.0),
          EventFactory.timed_event(summary: '緑3', color_id: EventFactory::GREEN, duration_hours: 1.5)
        ]
      end
      ...
      it 'categorizes, aggregates and sorts events by color correctly' do
        ...
        # 色別分類と集計
        expect(color_breakdown['緑']).to include(total_hours: 5.0, event_count: 3)
        ...
        expect(color_breakdown['オレンジ']).to include(total_hours: 1.0, event_count: 1)
        expect(color_breakdown['青']).to include(total_hours: 1.0, event_count: 1) # デフォルト色
        expect(color_breakdown['黄']).to include(total_hours: 0.0, event_count: 1) # ゼロ時間

        # 時間降順ソート
        color_names = subject[:color_breakdown].keys
        expect(color_names.first).to eq('赤')
        expect(color_names[1]).to eq('紫')
        expect(color_names[2]).to eq('緑')
    ...
    context 'with decimal precision requirements' do
      it 'rounds accumulated hours correctly' do
        ...
        expect(result[:color_breakdown]['緑'][:total_hours]).to eq(expected_total)
      ...
      it 'handles edge rounding cases correctly' do
        ...
        expect(result[:color_breakdown]['緑'][:total_hours]).to eq(0.13)
        expect(result[:color_breakdown]['青'][:total_hours]).to eq(0.00)
      end
    end
