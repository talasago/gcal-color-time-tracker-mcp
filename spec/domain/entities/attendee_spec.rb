...
describe Domain::Attendee do
  ...
  describe '#declined?' do
    context 'when response_status is declined' do
      let(:response_status) { 'declined' }

      it 'should return true' do
        expect(attendee.declined?).to be true
      end
    end

    context 'when response_status is not declined' do
      let(:response_status) { 'accepted' }

      it 'should return false' do
        expect(attendee.declined?).to be false
      end
    end
  end

  describe '#tentative?' do
    context 'when response_status is tentative' do
      let(:response_status) { 'tentative' }

      it 'should return true' do
        expect(attendee.tentative?).to be true
      end
    end

    context 'when response_status is not tentative' do
      let(:response_status) { 'accepted' }

      it 'should return false' do
        expect(attendee.tentative?).to be false
      end
    end
  end

  describe '#needs_action?' do
    context 'when response_status is needsAction' do
      let(:response_status) { 'needsAction' }

      it 'should return true' do
        expect(attendee.needs_action?).to be true
      end
    end

    context 'when response_status is not needsAction' do
      let(:response_status) { 'accepted' }

      it 'should return false' do
        expect(attendee.needs_action?).to be false
      end
    end
  end
end